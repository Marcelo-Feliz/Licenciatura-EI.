...
struct Dados *new_Dados2(int instante, struct Queue* RUN, int mem[MEM_SIZE])
{
    struct Dados *dados = malloc(sizeof(struct Dados)); // aloca espaco na memoria
...
struct PCB *new_PCB_BestFit(int mem[MEM_SIZE], struct Dados* auxiliar[ARRAY_SIZE], int id, int j, int bitmap[MEM_SIZE])
{   
...
struct PCB *new_PCB_NextFit(int mem[MEM_SIZE], struct Dados* auxiliar[ARRAY_SIZE], int id, int j, int bitmap[MEM_SIZE], int next_fit_count)
{
    struct PCB *processo = malloc(sizeof(struct PCB)); // aloca espaco na memoria
...
{   
    if(front(BLOCK) != NULL && BLOCK->size == 1)
    {   
    ...
    if(front(BLOCK) != NULL && BLOCK->size >= 2)
    {   
